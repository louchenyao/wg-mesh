...
def yaoyao():
    ...
    # client names
    clients = ["iPhone", "Macbook", "xzw", "lyh", "zhy", "qyx", "zys",
               "iPad", "lby", "zd-mac", "zd-phone", "zd-3", "zd-4", "gram", "wmd"]

    # setup hosts
    bj = Host("bj", bj_wan_ip, "10.56.100.1", "10.56.233.1",
              home="/root", key=Key(key_path="keys/bj.key"))
    hk = Host("hk", hk_wan_ip, "10.56.100.2", "10.56.233.2",
              home="/root", key=Key(key_path="keys/hk.key"))
    clients_hosts = []
    for d in clients:
        clients_hosts.append(
    ...
    bj.add_route(["10.56.100.0/24", "10.56.200.0/24", "10.56.233.0/24",
                  "10.56.40.0/24", "1.1.1.1", ], via=bj.lo_ns_ip, in_ns=False)
    ...
    bj.add_ipset(
        "https://pppublic.oss-cn-beijing.aliyuncs.com/ipsets.txt", in_ns=True)
    bj.add_iptable("mangle", "PREROUTING",
                   "-s 10.56.0.0/16"
                   + " -m set ! --match-set china_ip dst -m set ! --match-set private_ip dst"
                   + " -j MARK --set-xmark 0x1/0xffffffff", in_ns=True)
    bj.add_cmd(bj.ns_exec + "ip rule add fwmark 0x1 table 100")
    bj.add_cmd(bj.ns_exec + "ip route add default via %s table 100" %
               hk_bj.left_ip)

    ################################
    # hk                           #
    ################################
    hk.add_route(["10.56.100.0/24", "10.56.200.0/24", "10.56.233.0/24",
                  "10.56.40.0/24"], via=hk.lo_ns_ip, in_ns=False)
